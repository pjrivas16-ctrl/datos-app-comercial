...
export const calculateItemPrice = (item: QuoteState): number => {
    ...
    // Determine the effective product line for pricing, considering the "tapeta" extra for LUXE.
    let effectiveProductLine = productLine;
    const hasTapeta = extras?.some(e => e.id === 'tapeta-mismo-material');
    if (productLine === 'LUXE' && hasTapeta) {
        effectiveProductLine = 'LUXE CON TAPETA';
    }
    
    // Get base price from the price list using the effective product line.
    let basePrice = PRICE_LIST[effectiveProductLine]?.[width]?.[length] || 0;
